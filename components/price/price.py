...
# нужно организовать проверку что прайс не находится в заказах
# (и корзинах? в корзинах как проверить?), - никак - удалять
# карточка, к которой привязан прайс не удаляется
...
def delete_price(id):
    ...
    # удаляем заказы с этим прайсом из корзин пользователей
    ...
    for cart_user in carts_users:
        ...
        for order_request in cart_user['cart']:
            ...
            if order_request['price_id']!=price.id:
