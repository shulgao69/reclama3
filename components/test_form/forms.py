...
from wtforms import SubmitField, SelectField, HiddenField, BooleanField, StringField, \
    PasswordField, IntegerField, validators, DateField, DateTimeField, TelField
...
# *** Форма заявки 1 - конец


# *** Форма заявки 2  - начало
class TestForm2(FlaskForm):

    # ПРО регулярные выражения см TestForm1!!!
    user_phone = TelField("Введите телефон",
                          [InputRequired(), Regexp("^(\+)?(7|8)?(\s)*?(\-)?(\()?\d{3,5}(\))?(\s)*?(\-)?\d{1,3}(\s)*?(\-)?\d{2}(\s)*?(\-)?\d{2}(\s)*$",
                                                   message='Неверный формат телефона...')],
                          render_kw={'autofocus': True, 'placeholder': "+7(___)___-__-__" })
    ...
    # ПРО доп. валидатор см TestForm1!!!
...
# *** Форма заявки 2 - конец


# *** Форма заявки 3  - начало
